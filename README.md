...
To illustrate the K-nearest neighbors (KNN) algorithm I will take an easy example. Easy means with only 2 dimensions. This way, we will be able to display outputs on graphs without reducing the dimensions. It will be more pleasant to look at and therefore to interpret.

In this case, we have a dataset of average mark in maths and French of 300 French students during the year 2018. We also know what final exam they passed (Assuming they took only maths and French final exams at the end of the year and that we know the average marks only for those students, who had passed at least one of the exams). The goal of the KNN algorithm here is to predict what final exam a student of the next promotion will pass according to its maths and french average mark.
...
The KNN algoritmh is a method used for classification or regression. It consists of finding the k nearest training examples in the feature space. In our case the training examples are the French students and their features are their maths and French average mark. As I said there are two cases : 
* Classification : the output is a class. In our example the KNN is for classification and the output is one of the three classes (pass the French exam, pass the maths exam and pass both). The KNN algorithm finds the k closest training examples and the output is the class, which is majoritarily in those k neighbors. 
...
In both methods we can assign a ponderation to each neihgbor. As an example we can ponderate a neighbor in an inversely proportionnal way to its distance to the sample to predict.


### KNN in our problem

To predict what exam a student will pass according to its average marks and the training examples we will not use a ponderation according to the distances of the neighbors. As you may have understood, it is a KNN classification method here. A graph of the training point is shown below. Each student is represented as a point on a graph. This point has a x-coordinate according to its maths average mark and a y-coordinate according to its French average mark. The color of the training points means :
* Red : pass the final French exam
* Blue : pass the final maths exam
* Green : pass both

So for a new student (represented as a black point), who has 15 in maths and 12.75 in French, the 5-nearest neighbors are 3 green points, 1 red point and 1 blue point. Therefore 5-NN algorithm classify this student as a green point even if the blue point and the red points are closer than the green ones. So it predicts that this student will pass both exams.

![KNN explanation](/src/knn_explanation.png)

I took k = 5 in my example but how can we choose the k value ? 

To choose the best k value the easiest method is to test, which k value will have the best accuracy, for a range of k. In practice, the perfect k value does not exist. Especially since the data are condensed. It is impossible to reach a 100% accuracy for a k value. I ran 100 tests for each k from 1 to 150 as shown below. The best k are between 10 and 40. This selection method limits : 
* Overfitting : the predictive model will capture all the details that describe the data in the training set. In this case, it will capture all the noises and the random variations of the training set data. In other words, the predictive model will capture the generalizable correlations AND the noise produced by the data. 

As an example, for k < 3, the model is too specialized on the training set data and can not fit other given data.

* Underfitting : the predictive model can not even capture the correlations of the training set. As a result, the cost of error in the learning phase stays high.  Of course, the predictive model can not fit other given data.
...
For each k from 1 to N
    ...
    Test the accuracy of the k value for the test set according to the k-nearest neighbors in the training set
...
RUN_K_ACCURACY = False  # to display or not each k accuracy on a graph
...
Obvisouly, we need to load a dataset. The function `loadDataset()` take the .csv file name as argument and returns an array containing each row of the file, removing the headers.
...
```
Then the `toXY()` is not a useful function. It transposes the row into column to return each row element as an array. Moreover, it removes the last name and the first name columns. This way, in our case, the first returned array will be the maths one, then the French column and then the label column. This function takes the dataset as argument.

We could deal with the KNN algorithm without this function. Yet, I use it to give names for these arrays to be more explicit in my script. 
...
To start with the KNN algorithm we will first write the distance function. the `distance()` calculates the euclidean distance from a given point to each point of the training array and returns the associated array. It takes our arguments : params1 and params2, which will be respectively the maths and the French arrays of the training set, and it also takes the coordinates of a given point, which are the maths and French average marks.  
...
The function `findKNN()`, will find the k-nearest neighbors and return their row index. It takes five arguments : k is the number of neighbors to find and the last four arguments are the same as the previous function `distance()`. 
...
The `areaPrecditions()` function is the purpose of the KNN algorithm. It will calculate for each possible average mark the prediction made by the algorithm. As we can see on the graph above, the areas are divided in 3 colors. It basically means that if a point is on the blue area then the prediction is that the sample will pass the maths exam at the end of the year.
...
`step = 6` means that the square of an area equal to 1 would calculate the prediction for `step²` points, which would be 36 points to process. It can be easily changed.  
...
The `getAccuracy()` function simply calculates the accuracy of the predictions made by our algorithm on the test set considering the training set and according to a given k value. It takes three arguments : a training set, a test set to compare the prediction to the training samples and a given k value.   
...
The `choose_k()` function runs the previous function for a range of k values and returns the best one. It takes three arguments : the minimum k value to test, the maximum k value to test and the number of tests, which have to be done for each value.
...
All the functions are now operative. We only have to display the algorithm. The `display()` function is a main like function for the rendering. According to the `RUN_K_ACCURACY` and `RUN_AREA_PREDICTION` booleans.
